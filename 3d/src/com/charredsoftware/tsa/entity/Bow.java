...
public class Bow {
	...
	private void release(){
		...
		new Arrow(Main.getInstance().player, Main.getInstance().player.world, new Position(player.x, player.y + ((Main.getInstance().player.isCrouching) ? 0 : 1), player.z), drawBackTime, Main.getInstance().camera.rx, Main.getInstance().camera.ry);
		drawBackTime = 0f;
