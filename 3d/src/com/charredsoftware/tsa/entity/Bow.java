...
public class Bow {
	...
	public boolean UPGRADE_LARGER_RADIUS = false, UPGRADE_MORE_DAMAGE = false, UPGRADE_FURTHER_SHOTS = false;
	...
	private void release(){
		...
		new Arrow(Main.getInstance().player, Main.getInstance().player.world, new Position(player.x, player.y + ((Main.getInstance().player.isCrouching) ? 0 : 1), player.z), drawBackTime * 2, Main.getInstance().camera.rx, Main.getInstance().camera.ry);
		drawBackTime = 0f;
